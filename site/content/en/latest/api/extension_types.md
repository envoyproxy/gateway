...
| `Force` | MergeBackendsModeForce always merges a backend's cluster across routes, even when a<br />route-targeted BackendTrafficPolicy configures backend-cluster-scoped settings for it (those<br />settings are ignored for the shared cluster).<br />NOTE: Force mode is not implemented yet and currently behaves the same as BestEffort.<br /> | 

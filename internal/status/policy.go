...
func SetTranslationErrorForPolicyAncestors(policyStatus *gwv1a2.PolicyStatus, ancestorRefs []gwv1a2.ParentReference, controllerName string, generation int64, field, errMsg string) {
	message := fmt.Sprintf("Unable to translate %s: %s", field, errMsg)

	for _, ancestorRef := range ancestorRefs {
		SetConditionForPolicyAncestor(policyStatus, ancestorRef, controllerName,
			gwv1a2.PolicyConditionAccepted, metav1.ConditionFalse, gwv1a2.PolicyReasonInvalid, message, generation)
	}
}

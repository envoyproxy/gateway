...
| `ocspStaple` | _[SecretObjectReference](https://gateway-api.sigs.k8s.io/references/spec/#gateway.networking.k8s.io/v1.SecretObjectReference)_ |  false  |  | OCSPStaple references a Kubernetes ConfigMap or Secret containing the OCSP stapled response<br />that will be presented by Envoy during TLS handshakes.<br />The referenced object must contain a key named `ocsp.staple` with the<br />base64-encoded OCSP response.<br />Envoy Gateway also supports `ocsp.staple` in the certificateRefs of a Gateway Listener.<br />If both `OCSPStaple` in ClientTrafficPolicy and `ocsp.staple` in certificateRefs are configured,<br />the value from ClientTrafficPolicy takes precedence. |

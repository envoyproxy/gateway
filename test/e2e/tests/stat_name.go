...
var HTTPRouteStatNameTest = suite.ConformanceTest{
	...
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		...
		t.Run("prometheus", func(t *testing.T) {
			...
			// make sure listener is ready
			httputils.MakeRequestAndExpectEventuallyConsistentResponse(t, suite.RoundTripper, suite.TimeoutConfig, gwAddr, expectedResponse)
			verifyMetrics(t, suite, `envoy_vhost_route_upstream_rq{envoy_route="gateway-conformance-infra/http-route-stat-name/0"}`)
		})
	},
}
...
var TCPRouteStatNameTest = suite.ConformanceTest{
	...
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		...
		t.Run("prometheus", func(t *testing.T) {
			...
			// make sure listener is ready
			httputils.MakeRequestAndExpectEventuallyConsistentResponse(t, suite.RoundTripper, suite.TimeoutConfig, gwAddr, expectedResponse)
			verifyMetrics(t, suite, `envoy_tcp_downstream_cx_total{envoy_tcp_prefix="gateway-conformance-infra/tcp-route-stat-name/0"}`)
		})
	},
}

...
func testRouteToServicesMappings(t *testing.T, cache *providerReferenceStore) {
	...
	require.Equal(t, []types.NamespacedName{ns1svc1}, cache.getRouteToServicesMapping(httpr1))
	...
	require.Equal(t, []types.NamespacedName{ns1svc2}, cache.getRouteToServicesMapping(tlsr1))
	require.Equal(t, []types.NamespacedName{ns1svc1, ns1svc2}, cache.getRouteToServicesMapping(httpr1))

	// Remove HTTPRoute/ns1/r1 -> ns1/svc1 mapping
	cache.removeRouteToServicesMapping(httpr1, ns1svc1)
	require.Equal(t, []types.NamespacedName{ns1svc2}, cache.getRouteToServicesMapping(httpr1))

	// Remove TLSRoute/ns1/r1 -> ns1/svc2 mapping
	cache.removeRouteToServicesMapping(tlsr1, ns1svc2)
	require.Equal(t, []types.NamespacedName{}, cache.getRouteToServicesMapping(tlsr1))

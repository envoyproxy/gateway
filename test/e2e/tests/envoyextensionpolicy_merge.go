...
var EnvoyExtensionPolicyMergedTest = suite.ConformanceTest{
	...
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		...
		t.Run("route policy merges with listenerset policy", func(t *testing.T) {
			ns := "gateway-conformance-infra"
			gwNN := types.NamespacedName{Name: "eep-merge-policy-gateway", Namespace: ns}
			lsNN := types.NamespacedName{Name: "eep-merge-policy-ls", Namespace: ns}
			routeNN := types.NamespacedName{Name: "eep-merge-policy-route", Namespace: ns}
			...
			EnvoyExtensionPolicyMustBeAccepted(t, suite.Client,
				types.NamespacedName{Name: "eep-merge-listenerset", Namespace: ns},
				suite.ControllerName, lsAncestorRef)
			EnvoyExtensionPolicyMustBeMerged(t, suite.Client,
				types.NamespacedName{Name: "eep-merge-listenerset", Namespace: ns},
				suite.ControllerName, lsAncestorRef)
			EnvoyExtensionPolicyMustBeMerged(t, suite.Client,
				types.NamespacedName{Name: "eep-merge-route", Namespace: ns},
				suite.ControllerName, routeAncestorRef)
			...
			expectedResponse := httputils.ExpectedResponse{
				...
				Response: httputils.Response{
					StatusCodes: []int{200},
					Headers: map[string]string{
						"X-ListenerSet-Merged":     "true",
						"x-response-ext-processed": "true",
					},
				},
				Namespace: ns,
			}
			httputils.MakeRequestAndExpectEventuallyConsistentResponse(t, suite.RoundTripper, suite.TimeoutConfig, getListenerAddr(gwAddr, "18198"), expectedResponse)
		})
	},
}

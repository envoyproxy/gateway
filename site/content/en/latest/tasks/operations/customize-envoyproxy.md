...
The built-in [HPA](#customize-envoyproxy-horizontal-pod-autoscaler) only supports what a native
`HorizontalPodAutoscaler` metrics spec can express. To scale the proxy fleet with an autoscaler that Envoy Gateway
does not manage (a user-managed `HorizontalPodAutoscaler`, or a [KEDA](https://keda.sh/) `ScaledObject` scaling on a
Prometheus query or another event source), **leave `envoyDeployment.replicas` unset**.

Envoy Gateway only sets `spec.replicas` on the generated Deployment when you explicitly configure
[`envoyDeployment.replicas`](#customize-envoyproxy-deployment-replicas). When the field is unset it is omitted from the
server-side apply patch, so Envoy Gateway never takes ownership of it and will not revert the replica count computed by
your autoscaler on the next reconciliation. No additional Envoy Gateway configuration is required.

{{% alert title="Migrating from an explicit replica count" color="warning" %}}
If you previously set `envoyDeployment.replicas` and then remove it to hand the field to an external autoscaler,
server-side apply removes `spec.replicas` from the Deployment and the API server re-defaults it to `1`. Expect a
scale-down until your autoscaler reconciles. Create the autoscaler before removing the field, or scale the Deployment
back up manually.
{{% /alert %}}

### Targeting the generated Deployment

An external autoscaler needs a stable reference to the proxy Deployment. By default the name is autogenerated from the
owning Gateway, so use one of the following instead of recomputing it:

- **Pin the name**: set `envoyDeployment.name` to a fixed value and use it as your autoscaler's `scaleTargetRef.name`.
- **Select by label**: the generated Deployment carries `gateway.envoyproxy.io/owning-gateway-name` and
  `gateway.envoyproxy.io/owning-gateway-namespace`, or, when `mergeGateways` is enabled,
  `gateway.envoyproxy.io/owning-gatewayclass`.

Pin the Deployment name in the EnvoyProxy:
...
Then point the autoscaler at that name. A minimal KEDA example:
...
To confirm that Envoy Gateway is not managing the replica count, check that it is not listed as a field manager for
`spec.replicas`. The following command prints the managers of that field, and returns nothing when Envoy Gateway has
not claimed it:

```shell
kubectl get deployment my-envoy-proxy -n envoy-gateway-system --show-managed-fields -o yaml | grep -B5 'f:replicas'
```

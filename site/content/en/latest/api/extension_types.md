...
| `compressor` | _[Compression](#compression) array_ |  false  |  | The compressor config for the http streams.<br />This provides more granular control over compression configuration.<br />Order matters: The first compressor in the list is preferred when q-values in Accept-Encoding are equal.<br />Each entry must configure the codec matching its type, for example, type: Gzip with gzip: \{\}.<br />A merge policy may omit the matching codec configuration to disable an inherited compressor. |

...
| `clientTLS` | _[BackendTLSConfig](#backendtlsconfig)_ |  false  |  | ClientTLS defines the client TLS settings that Envoy will use when initiating<br />TLS connections to this backend. This allows configuring the client certificate/key<br />as well as TLS protocol parameters such as ciphers, TLS versions, and ALPN.<br />When omitted, Envoy will fall back to the EnvoyProxy BackendTLS defaults, if any. |

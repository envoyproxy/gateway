...
spec:
...
local validation error: Gateway.gateway.networking.k8s.io "eg2" is invalid: [spec.listeners[1]: Duplicate value: {"name":"tcp"}, spec.listeners: Invalid value: Listener name must be unique within the Gateway, spec.listeners: Invalid value: Combination of port, protocol and hostname must be unique for each listener]
...
spec:
...
local validation error: Backend.gateway.envoyproxy.io "backend-2" is invalid: spec.endpoints: Invalid value: fqdn addresses cannot be mixed with other address types

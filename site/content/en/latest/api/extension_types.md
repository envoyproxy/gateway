...
| `frontendTLS` | _[FrontendTLSConfig](#frontendtlsconfig)_ |  false  |  | FrontendTLS is the TLS configuration for the Envoy proxy to use when receiving connections from clients.<br />These settings are applied on listeners for which TLS termination is specified. |
...
| Field | Type | Required | Default | Description |
| ---   | ---  | ---      | ---     | ---         |
| `skipHostnameValidation` | _boolean_ |  false  | false | SkipHostnameValidation, if true, will instruct the translator to not validate<br />referenced certificates will match the hostname of an associated listener. |

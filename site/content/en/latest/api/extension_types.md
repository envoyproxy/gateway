...
| `spanName` | _[TracingSpanName](#tracingspanname)_ |  false  |  | SpanName defines the name of the span which will be used for tracing.<br />If not set, the span name is provider specific.<br />e.g. Datadog use `ingress` as the default ingress span name,<br />and `router <UPSTREAM_CLUSTER> egress` as the egress span name. |

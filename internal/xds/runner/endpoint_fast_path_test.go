...
func (f *fakeCache) UpdateEndpointResources(irKey string, assignments []envoytypes.Resource, _ context.Context) (bool, error) {
	if f.patchErr != nil {
		if f.patchCommitted {
			f.patches = append(f.patches, patchCall{irKey: irKey, assignments: assignments})
		}
		return f.patchCommitted, f.patchErr
	}
	f.patches = append(f.patches, patchCall{irKey: irKey, assignments: assignments})
	return true, nil
}
...
func TestFastPathKeepsContextWhenPatchCommittedWithError(t *testing.T) {
	...
	// cached context must keep describing the live snapshot: reverting it
	// would make a later rebuild of the same cluster (e.g. triggered by
	// another backend it serves) regress these endpoints.
...
func TestFastPathRevertsContextOnPatchFailure(t *testing.T) {
	fc := &fakeCache{}
	fp := testFastPath(fc)

	// Nothing was committed, so the cached context must roll back and
	// replaying the same update must not be mistaken for applied state.
	fc.patchErr = context.DeadlineExceeded
	fp.handleUpdate(testUpdate("2.2.2.2"))
	require.Empty(t, fc.patches)
	...
	fp.handleUpdate(testUpdate("2.2.2.2"))
	require.Len(t, fc.patches, 1)
	require.ElementsMatch(t, []string{"2.2.2.2"}, claAddresses(t, fc.patches[0].assignments[0]))
}

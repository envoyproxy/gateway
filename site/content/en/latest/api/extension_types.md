...
| `patchType` | _[MergeType](#mergetype)_ |  false  |  | PatchType determines how this configuration is merged with existing BackendTrafficPolicy<br />configurations targeting a parent resource. When set, this configuration will be merged<br />into a parent BackendTrafficPolicy (i.e. the one targeting a Gateway or Listener).<br />This field cannot be set when targeting a parent resource (Gateway).<br />If unset, no patching occurs, and only the most specific configuration takes effect. |

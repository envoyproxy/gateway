	...
	message.HandleSubscription(message.Metadata{Runner: r.Name(), Message: message.ProviderResourcesMessageName}, sub,
		func(update message.Update[string, *resource.ControllerResources], errChan chan error) {
			...
			aggregatedStatuses := struct {
				Backends                map[types.NamespacedName]*egv1a1.BackendStatus
				Gateways                map[types.NamespacedName]*gwapiv1.GatewayStatus
				...
				Backends:                make(map[types.NamespacedName]*egv1a1.BackendStatus),
				Gateways:                make(map[types.NamespacedName]*gwapiv1.GatewayStatus),
			...
			for _, resources := range *val {
				...
				for _, gateway := range result.Gateways {
					aggregatedStatuses.Gateways[utils.NamespacedName(gateway)] = &gateway.Status
				}
				for _, httpRoute := range result.HTTPRoutes {
					if len(httpRoute.Status.Parents) != 0 {
						key := utils.NamespacedName(httpRoute)
						aggregatedStatuses.HTTPRoutes[key] = append(aggregatedStatuses.HTTPRoutes[key], &httpRoute.Status.RouteStatus)
					}
				}
				for _, grpcRoute := range result.GRPCRoutes {
					if len(grpcRoute.Status.Parents) != 0 {
						key := utils.NamespacedName(grpcRoute)
						aggregatedStatuses.GRPCRoutes[key] = append(aggregatedStatuses.GRPCRoutes[key], &grpcRoute.Status.RouteStatus)
					}
				}
				for _, tlsRoute := range result.TLSRoutes {
					if len(tlsRoute.Status.Parents) != 0 {
						key := utils.NamespacedName(tlsRoute)
						aggregatedStatuses.TLSRoutes[key] = append(aggregatedStatuses.TLSRoutes[key], &tlsRoute.Status.RouteStatus)
					}
				}
				for _, tcpRoute := range result.TCPRoutes {
					if len(tcpRoute.Status.Parents) != 0 {
						key := utils.NamespacedName(tcpRoute)
						aggregatedStatuses.TCPRoutes[key] = append(aggregatedStatuses.TCPRoutes[key], &tcpRoute.Status.RouteStatus)
					}
				}
				for _, udpRoute := range result.UDPRoutes {
					if len(udpRoute.Status.Parents) != 0 {
						key := utils.NamespacedName(udpRoute)
						aggregatedStatuses.UDPRoutes[key] = append(aggregatedStatuses.UDPRoutes[key], &udpRoute.Status.RouteStatus)
					}
				}
				for _, backendTLSPolicy := range result.BackendTLSPolicies {
					if len(backendTLSPolicy.Status.Ancestors) != 0 {
						...
						aggregatedStatuses.BackendTLSPolicies[key] = append(aggregatedStatuses.BackendTLSPolicies[key], &backendTLSPolicy.Status)
					}
				}
				for _, clientTrafficPolicy := range result.ClientTrafficPolicies {
					if len(clientTrafficPolicy.Status.Ancestors) != 0 {
						...
						aggregatedStatuses.ClientTrafficPolicies[key] = append(aggregatedStatuses.ClientTrafficPolicies[key], &clientTrafficPolicy.Status)
					}
				}
				for _, backendTrafficPolicy := range result.BackendTrafficPolicies {
					if len(backendTrafficPolicy.Status.Ancestors) != 0 {
						...
						aggregatedStatuses.BackendTrafficPolicies[key] = append(aggregatedStatuses.BackendTrafficPolicies[key], &backendTrafficPolicy.Status)
					}
				}
				for _, securityPolicy := range result.SecurityPolicies {
					if len(securityPolicy.Status.Ancestors) != 0 {
						...
						aggregatedStatuses.SecurityPolicies[key] = append(aggregatedStatuses.SecurityPolicies[key], &securityPolicy.Status)
					}
				}
				for _, envoyExtensionPolicy := range result.EnvoyExtensionPolicies {
					if len(envoyExtensionPolicy.Status.Ancestors) != 0 {
						...
						aggregatedStatuses.EnvoyExtensionPolicies[key] = append(aggregatedStatuses.EnvoyExtensionPolicies[key], &envoyExtensionPolicy.Status)
					}
				}
				for _, extServerPolicy := range result.ExtensionServerPolicies {
					...
					if len(status.Ancestors) != 0 {
						...
						aggregatedStatuses.ExtensionServerPolicies[key] = append(aggregatedStatuses.ExtensionServerPolicies[key], &status)
					}
				}
				for _, backend := range result.Backends {
					aggregatedStatuses.Backends[utils.NamespacedName(backend)] = &backend.Status
				}
			}

			aggregateRouteStatus := func(routeStatuses []*gwapiv1.RouteStatus) gwapiv1.RouteStatus {
				var status gwapiv1.RouteStatus
				for _, routeStatus := range routeStatuses {
					status.Parents = append(status.Parents, routeStatus.Parents...)
				}
				return status
			}
			aggregatePolicyStatus := func(policyStatuses []*gwapiv1.PolicyStatus) gwapiv1.PolicyStatus {
				var status gwapiv1.PolicyStatus
				for _, policyStatus := range policyStatuses {
					status.Ancestors = append(status.Ancestors, policyStatus.Ancestors...)
				}
				return status
			}
			// Store the stauses of all objects atomically with the aggregated status.
			for key, status := range aggregatedStatuses.Gateways {
				r.ProviderResources.GatewayStatuses.Store(key, status)
				gatewayStatusCount++
				delete(keysToDelete.GatewayStatus, key)
				r.keyCache.GatewayStatus[key] = true
			}
			for key, statuses := range aggregatedStatuses.HTTPRoutes {
				status := gwapiv1.HTTPRouteStatus{RouteStatus: aggregateRouteStatus(statuses)}
				r.ProviderResources.HTTPRouteStatuses.Store(key, &status)
				httpRouteStatusCount++
				delete(keysToDelete.HTTPRouteStatus, key)
				r.keyCache.HTTPRouteStatus[key] = true
			}
			for key, statuses := range aggregatedStatuses.GRPCRoutes {
				status := gwapiv1.GRPCRouteStatus{RouteStatus: aggregateRouteStatus(statuses)}
				r.ProviderResources.GRPCRouteStatuses.Store(key, &status)
				grpcRouteStatusCount++
				delete(keysToDelete.GRPCRouteStatus, key)
				r.keyCache.GRPCRouteStatus[key] = true
			}
			for key, statuses := range aggregatedStatuses.TLSRoutes {
				status := gwapiv1a2.TLSRouteStatus{RouteStatus: aggregateRouteStatus(statuses)}
				r.ProviderResources.TLSRouteStatuses.Store(key, &status)
				tlsRouteStatusCount++
				delete(keysToDelete.TLSRouteStatus, key)
				r.keyCache.TLSRouteStatus[key] = true
			}
			for key, statuses := range aggregatedStatuses.TCPRoutes {
				status := gwapiv1a2.TCPRouteStatus{RouteStatus: aggregateRouteStatus(statuses)}
				r.ProviderResources.TCPRouteStatuses.Store(key, &status)
				tcpRouteStatusCount++
				delete(keysToDelete.TCPRouteStatus, key)
				r.keyCache.TCPRouteStatus[key] = true
			}
			for key, statuses := range aggregatedStatuses.UDPRoutes {
				status := gwapiv1a2.UDPRouteStatus{RouteStatus: aggregateRouteStatus(statuses)}
				r.ProviderResources.UDPRouteStatuses.Store(key, &status)
				udpRouteStatusCount++
				delete(keysToDelete.UDPRouteStatus, key)
				r.keyCache.UDPRouteStatus[key] = true
			}
			for key, statuses := range aggregatedStatuses.BackendTLSPolicies {
				status := aggregatePolicyStatus(statuses)
			...
			for key, statuses := range aggregatedStatuses.ClientTrafficPolicies {
				status := aggregatePolicyStatus(statuses)
			...
			for key, statuses := range aggregatedStatuses.BackendTrafficPolicies {
				status := aggregatePolicyStatus(statuses)
			...
			for key, statuses := range aggregatedStatuses.SecurityPolicies {
				status := aggregatePolicyStatus(statuses)
			...
			for key, statuses := range aggregatedStatuses.EnvoyExtensionPolicies {
				status := aggregatePolicyStatus(statuses)
			...
			for key, statuses := range aggregatedStatuses.ExtensionServerPolicies {
				status := aggregatePolicyStatus(statuses)
			...
			for key, status := range aggregatedStatuses.Backends {
				if len(status.Conditions) > 0 {
					r.ProviderResources.BackendStatuses.Store(key, status)
					backendStatusCount++
				}

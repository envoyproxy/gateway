...
sync-gwapi-docs: gwapi-doc-download gwapi-doc-transform gwapi-download-includes gwapi-replace-includes gwapi-clean-includes gwapi-doc-sync-images gwapi-doc-update-image-paths gwapi-doc-update-relative-links gwapi-doc-remove-special-lines
...
gwapi-clean-includes:
	@$(LOG_TARGET)
	@find $(DOC_DEST_DIR) -name '*.yaml' -exec rm {} +

# Download images referenced in the documentation and save them to the local image directory.
gwapi-doc-sync-images:
	@$(LOG_TARGET)
	@$(foreach file, $(SYNC_FILES), \
		sed -n 's/!\[.*\](\/images\/\(.*\))/\1/p' $(DOC_DEST_DIR)/$(file) | \
		xargs -I {} curl -s -o $(IMAGE_DEST_DIR)/{} $(IMAGE_SRC_BASE_URL)/{};)

# Update image paths in the documentation to reflect the new local paths.
gwapi-doc-update-image-paths:
	@$(LOG_TARGET)
	@$(foreach file, $(SYNC_FILES), \
		sed -i '' 's/\/images\//\/img\//g' $(DOC_DEST_DIR)/$(file);)

# Update relative links in the documentation to be absolute URLs pointing to the gateway-api website.
gwapi-doc-update-relative-links:
	@$(LOG_TARGET)
	@$(foreach file, $(SYNC_FILES), \
		sed -i '' -e 's/\([^!]\)\(\[.*\]\)(\(\/[^:]*\))/\1\2(https:\/\/gateway-api.sigs.k8s.io\3)/g' -e 's/\(\[.*\]: \)\(\/[^:]*\)/\1https:\/\/gateway-api.sigs.k8s.io\2/g' $(DOC_DEST_DIR)/$(file);)

# Remove special lines that start with '!!!' from the documentation.
...
docs-api-gen:
	...
	--output-path=site/content/en/latest/api/envoygateway/extension_types.md \
	--max-depth 100 \
	--renderer=markdown

...
func buildXdsClusterLoadAssignment(clusterName string, destSettings []*ir.DestinationSetting, preferLocal *ir.PreferLocalZone, weightedZones []ir.WeightedZoneConfig) *endpointv3.ClusterLoadAssignment {
	...
	for i, ds := range destSettings {
		...
		case len(weightedZones) > 0:
			localities = append(localities, buildWeightedZonalLocalities(metadata, ds, weightedZones)...)
		default:
...
func buildWeightedZonalLocalities(metadata *corev3.Metadata, ds *ir.DestinationSetting, weightedZones []ir.WeightedZoneConfig) []*endpointv3.LocalityLbEndpoints {

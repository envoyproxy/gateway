		...
		t.Run("scaling up httproutes", func(t *testing.T) {
			...
			for _, scale := range routeScales {
				...
				t.Run(testName, func(t *testing.T) {
					...
					report, err := bSuite.Benchmark(t, ctx, jobName, testName, gatewayAddr, routeHostnameFormat, int(totalHosts), time.Now())
					require.NoError(t, err)
		...
		t.Run("scaling down httproutes", func(t *testing.T) {
			...
			for i := routeScalesN - 2; i >= 0; i-- {
				...
				t.Run(testName, func(t *testing.T) {
					...
					report, err := bSuite.Benchmark(t, ctx, jobName, testName, gatewayAddr, routeHostnameFormat, int(totalHosts), startAt)
					require.NoError(t, err)
...
func getRouteConvergenceDuration(durations map[string]time.Duration) suite.PerfDuration {
	...
	p99 := stat.Quantile(0.99, stat.Empirical, weights, nil)
	fmt.Printf("p99: %.2f\n", p99)

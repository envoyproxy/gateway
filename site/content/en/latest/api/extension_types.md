...
| `reuseConnection` | _boolean_ |  false  |  | ReuseConnection determines whether the active health check connection to a backend host<br />is reused across health check probes. When set to false, Envoy opens a new connection<br />for each health check, which is useful for backends (such as haproxy-style agent checks)<br />that do not expect a reused connection.<br />Defaults to true. |

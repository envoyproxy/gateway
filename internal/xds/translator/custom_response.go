...
func(c *customResponse) buildSinglePredicate(codeMatch ir.StatusCodeMatch) (*matcherv3.Matcher_MatcherList_Predicate, error) {
	...
	if codeMatch.Range != nil {
		...
		if statusCodeCELMatcher, err = c.buildStatusCodeCELMatcher(codeMatch); err != nil {
			return nil, err
		}
...
// Example of the CEL expression AST:
/* {
	"id":9,
	"ExprKind":{
	   "CallExpr":{
...
func(c *customResponse) buildStatusCodeCELMatcher(codeMatch ir.StatusCodeMatch) (*cncfv3.TypedExtensionConfig, error) {
	var (
		pb  *anypb.Any
		err error
	)

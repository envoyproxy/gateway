...
// caPEM is one self-signed CA, repeated to build bundles of a realistic size.
...
// caConfigMapYAML is a single CA bundle shared by every generated BackendTLSPolicy,
// mirroring the common deployment where many backends validate against one corporate
// CA. Sized to match what production profiles show, since the cost being measured is
// proportional to the bundle size.
const caConfigMapYAML = `---
apiVersion: v1
kind: ConfigMap
metadata:
  name: shared-ca
...
func BenchmarkXdsIRDeepCopy(b *testing.B) {
	caBundle := []byte(strings.Repeat(caPEM, 20)) // ~22KB, the size production profiles show

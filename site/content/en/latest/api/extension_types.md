...
| `labels` | _object (keys:LabelKey, values:LabelValue)_ |  false  |  | Labels are the additional labels that should be tagged to the pods.<br />By default, no additional pod labels are tagged.<br />Keys must be valid label keys (optionally prefixed qualified names) and<br />values must satisfy the Kubernetes label value format, otherwise the<br />resource is rejected. |
...
| `labels` | _object (keys:LabelKey, values:LabelValue)_ |  false  |  | Labels that should be appended to the service.<br />By default, no labels are appended.<br />Keys must be valid label keys (optionally prefixed qualified names) and<br />values must satisfy the Kubernetes label value format, otherwise the<br />resource is rejected. |

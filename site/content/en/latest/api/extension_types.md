...
| Field | Type | Required | Default | Description |
| ---   | ---  | ---      | ---     | ---         |
| `enforcedFraction` | _[Fraction](https://gateway-api.sigs.k8s.io/reference/api-spec/1.5/spec/#fraction)_ |  false  |  | EnforcedFraction represents the fraction of requests for which the CSRF<br />policy is enforced. Requests that are not selected are allowed through<br />without any origin validation.<br />Defaults to 100% (all requests are enforced) if not specified. |
| `shadowFraction` | _[Fraction](https://gateway-api.sigs.k8s.io/reference/api-spec/1.5/spec/#fraction)_ |  false  |  | ShadowFraction represents the fraction of requests for which the CSRF<br />policy is evaluated in shadow (dry-run) mode. In this mode, the filter<br />evaluates requests and tracks whether they would be allowed or rejected in<br />the `csrf.request_invalid` and `csrf.request_valid` stats, but does not<br />enforce the policy. This is useful for rolling out CSRF protection<br />gradually while monitoring the impact.<br />Only takes effect for requests that are not selected by EnforcedFraction. |

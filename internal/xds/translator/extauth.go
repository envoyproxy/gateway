...
func (*extAuth) patchResources(tCtx *types.ResourceVersionTable,
	...
	for _, route := range routes {
		...
		if http := route.Security.ExtAuth.HTTP; http != nil {
			...
			if err := processClientCertificates(tCtx, tCtx.GetBackendClusters(&http.Destination)); err != nil {
				errs = errors.Join(errs, err)
			}
		} else {
			...
			if err := processClientCertificates(tCtx, tCtx.GetBackendClusters(&grpc.Destination)); err != nil {
				errs = errors.Join(errs, err)
			}
		}
	}

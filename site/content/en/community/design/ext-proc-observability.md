...
| 6 | **Specificity-then-age wins per IR listener** via a `nameRegistry`; route-rule > route > listener > gateway, oldest within the same scope. Shadowed policies get `CustomName` cleared and an `AmbiguousDefinition` warning; the effective policy's filter identity applies across the entire listener. Under `MergeGateways` without TLS, `name` is suppressed (xDS listener order follows gateway timestamps, independent of EEP age). With TLS each listener has its own filter chain and registry. | Rejecting all duplicates (rejected: breaks the oldest, authoritative policy). |

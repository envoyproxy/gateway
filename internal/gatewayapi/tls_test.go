...
func TestValidateTLSSecretsData(t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			Name:                   "valid-rsa-san-domain-skip-validation",
			CertFile:               "rsa-cert-san.pem",
			KeyFile:                "rsa-pkcs8-san.key",
		...
		{
			Name:                   "valid-rsa-wildcard-domain-skip-validation",
			CertFile:               "rsa-cert-wildcard.pem",
			KeyFile:                "rsa-pkcs1-wildcard.key",
			Domain:                 "foo.bar-baz.com",
			SkipHostnameValidation: true,
	...
	for _, tc := range testCases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			_, err := validateTLSSecretsData(secrets, &tc.Domain, tc.SkipHostnameValidation)

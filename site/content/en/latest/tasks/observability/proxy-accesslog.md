...
> **Note:** Names are not validated for uniqueness across policies — duplicates can arise from multiple `EnvoyExtensionPolicy` resources targeting routes on the same listener. Within a single listener, Envoy Gateway resolves `%EG_EXT_FILTER_STATE(name:attribute)%` by **oldest policy wins**: the `EnvoyExtensionPolicy` with the oldest creation timestamp that claims a name is the winner. Newer policies that reuse the same name receive a `Warning` condition with reason `AmbiguousDefinition` and their operator will not resolve. The routes themselves still execute their own ext-proc filters normally. Use distinct names unless you are certain the routes are on isolated filter chains (see below).
...
> **MergeGateways (non-TLS):** When [`MergeGateways`](https://gateway.envoyproxy.io/docs/api/extension_types/#mergegatewaysconfig) is enabled, non-TLS listeners on the same port share a single HCM across all merged gateways. In this case the isolated-listener guarantee does not hold. Name collisions across gateways are resolved silently — **no warning condition is issued**. The winning instance belongs to the oldest merged gateway (by creation timestamp), processed first. Newer gateways' ext-proc instances with the same name are ignored for access log expansion. Use distinct names across merged gateways, or ensure only one gateway registers a given name.

...
type Xds struct {
	...
	// HealthCheckLog holds gateway-level HC event logging config (EnvoyProxy.spec.telemetry.healthCheckLog).
...
type ActiveHealthCheck struct {
	...
	// EventLog configures HC event logging for this cluster (BTP path via ClusterSettings).
	...
	// Takes precedence over the gateway-level HealthCheckLog for this specific cluster.
...
// ProxyHealthCheckLog configures Envoy health check event logging for a cluster.
// +k8s:deepcopy-gen=true
type ProxyHealthCheckLog struct {
	...
	// AlwaysLogHealthCheckFailures maps to Envoy's always_log_health_check_failures.
	...
	// AlwaysLogHealthCheckSuccess maps to Envoy's always_log_health_check_success.
	AlwaysLogHealthCheckSuccess bool `json:"alwaysLogHealthCheckSuccess,omitempty" yaml:"alwaysLogHealthCheckSuccess,omitempty"`
}

// FileEnvoyProxyHealthCheckLog writes HC events as JSON to a file path.
// +k8s:deepcopy-gen=true
type FileEnvoyProxyHealthCheckLog struct {
	// Path is the file path (e.g. /dev/stdout).
	Path string `json:"path" yaml:"path"`
}

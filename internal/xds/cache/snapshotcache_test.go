...
func newTestSnapshotCache(t *testing.T) *snapshotCache {
	...
	cache := NewSnapshotCache(false, true, logger)
...
func TestUpdateEndpointResources(t *testing.T) {
	...
	// Populate the delta version map so carry-forward is exercised.
	require.NoError(t, oldSnapshot.ConstructVersionMap())

	// Patching an unknown IR key is a no-op.
	patched, err := sc.UpdateEndpointResources("unknown", []envoytypes.Resource{cla("cluster-a", "3.3.3.3")}, context.Background())
	require.NoError(t, err)
	require.False(t, patched)

	patched, err = sc.UpdateEndpointResources(irKey, []envoytypes.Resource{cla("cluster-a", "3.3.3.3")}, context.Background())
	require.NoError(t, err)
	...
	// The delta version map is carried forward for unchanged types and
	// re-hashed for EDS: cluster-a's hash changed, cluster-b's did not.
	require.Equal(t, oldSnapshot.GetVersionMap(resourcev3.ClusterType), newSnapshot.GetVersionMap(resourcev3.ClusterType))

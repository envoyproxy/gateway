...
// without waiting for a full translation. The full reconcile enqueued for the
// same event remains authoritative; this is a best-effort fast path.
func (r *gatewayAPIReconciler) publishEndpointUpdate(ctx context.Context, eps *discoveryv1.EndpointSlice) {
	...
	// Match the full path: use the field index when it is enabled, and fall
	// back to the label otherwise.
	...
	for i := range endpointSliceList.Items {
		slice := &endpointSliceList.Items[i]
		// The index matches on one label, but a slice carrying both belongs to
		// the Service alone. Applying the same precedence here keeps the set
		// identical to the one the translation derives for this backend.
		if sliceKind, _, ok := endpointsutil.BackendForSlice(slice); !ok || sliceKind != kind {
			continue
		}
		update.EndpointSlices = append(update.EndpointSlices, slice)
	}
	...
	message.PublishMetric(message.Metadata{
		...
		Message: message.EndpointUpdatesMessageName,
	}, 1)
}

...
# KUBE_TEST_PACKAGE=./internal/provider/kubernetes/... make run-kube-test
KUBE_TEST_PACKAGE ?= ./...
...
run-kube-test: # Run Kubernetes provider tests.
	@$(LOG_TARGET)
	KUBEBUILDER_ASSETS="$(shell $(GO_TOOL) setup-envtest use $(ENVTEST_K8S_VERSION) -p path)" go test $(KUBE_TEST_ARGS) --tags=$(KUBE_TEST_TAGS) $(KUBE_TEST_PACKAGE) -coverprofile cover.out

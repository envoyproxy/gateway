...
We can specify to install only workload resources via `--skip-crd`

```bash
egctl x install --skip-crd
```

We can specify to install only CRDs resources via `--only-crd`

```bash
egctl x install --only-crd
```

We can specify `--release-name` and `--namespace` to install envoy-gateway in different places to support multi-tenant mode. 
> Note: If CRDs are already installed, then we need to specify `--skip-crd` to avoid repeated installation of CRDs resources.
...
By default, this will only uninstall the envoy-gateway workload resource, if we want to also uninstall CRDs, we need to specify `--with-crd`

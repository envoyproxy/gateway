...
func buildEndpointType(settings []*ir.DestinationSetting) EndpointType {
	...
	if settings[0].IsStaticResolver {
		return EndpointTypeStaticResolver
	}
...
func buildXdsCluster(args *xdsClusterArgs) (*buildClusterResult, error) {
	...
	case EndpointTypeStaticResolver:
		// Static resolver uses EDS for endpoints but with override host load balancing policy
		...
		// Load balancing policy will be set to override host in buildStaticResolverLoadBalancingPolicy
	...
	// Set override host load balancing policy for static resolver clusters
	if args.endpointType == EndpointTypeStaticResolver {
		if err := buildStaticResolverLoadBalancingPolicy(cluster, args.settings); err != nil {
			return nil, err
		}
	}
...
// buildStaticResolverLoadBalancingPolicy configures an xds cluster with Override Host Load Balancing Policy
// for static resolver backends. This allows endpoint selection based on request headers or dynamic metadata.
// The actual endpoints will be provided at runtime through service discovery or other mechanisms.
func buildStaticResolverLoadBalancingPolicy(cluster *clusterv3.Cluster, settings []*ir.DestinationSetting) error {
	if len(settings) == 0 || !settings[0].IsStaticResolver || settings[0].StaticResolverConfig == nil {
		return fmt.Errorf("invalid static resolver configuration")
	}

	config := settings[0].StaticResolverConfig
	...
	// Build fallback policy (default to round robin)
	roundRobin := &round_robinv3.RoundRobin{}
	typedRoundRobin, err := anypb.New(roundRobin)
	if err != nil {
		return fmt.Errorf("failed to marshal round robin policy: %w", err)
	}

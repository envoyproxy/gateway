...
func (t *Translator) processHTTPListenerXdsTranslation(
	...
	for _, httpListener := range httpListeners {
		...
		if addFilterChain {
			...
			hcm, err := findHCMinFilterChain(xdsListener.DefaultFilterChain)
			if err != nil {
			...
			if err = updateHCMinFilterChain(xdsListener.DefaultFilterChain, hcm); err != nil {
				return err
			}
		}
...
func updateHCMinFilterChain(filterChain *listenerv3.FilterChain, hcm *hcmv3.HttpConnectionManager) error {
	for i, filter := range filterChain.Filters {
		if filter.Name == wellknown.HTTPConnectionManager {
			mgrAny, err := protocov.ToAnyWithError(hcm)
			if err != nil {
				return err
			}

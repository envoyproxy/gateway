...
type SourceMatch struct {
	...
	// Note that for Exact type (shared bucket), same descriptor value is used irrespective of whether invert match is set to true. For example,
	// if the CIDR is 192.168.0.0/24 and the source IP is 10.1.0.2 with invert match set to true, a descriptor with value 192.168.0.0/24 will be used.
	//
	// +kubebuilder:validation:Required
	// +kubebuilder:validation:MinLength=1
	// +kubebuilder:validation:MaxLength=256

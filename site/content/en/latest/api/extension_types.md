...
| `vmID` | _string_ |  false  |  | VMID is the ID of the Wasm VM that runs this extension. Extensions with the<br />same VMID and the same Wasm code share a single VM, which saves the memory a<br />separate VM per extension would use.<br />If not specified, EG generates a VM ID that is unique to this extension, so<br />extensions never share a VM.<br />Note: extensions sharing a VM also share its global state, and the VM is started<br />with the configuration of the first extension that initializes it. |

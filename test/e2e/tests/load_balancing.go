...
var HostOverrideLoadBalancingTest = suite.ConformanceTest{
	ShortName:   "HostOverrideLoadBalancing",
	Description: "Test for host override load balancing type",
	Manifests:   []string{"testdata/load_balancing_host_override.yaml"},
	Test: func(t *testing.T, suite *suite.ConformanceTestSuite) {
		const sendRequests = 10

		ns := "gateway-conformance-infra"
		headerRouteNN := types.NamespacedName{Name: "host-override-header-lb-route", Namespace: ns}
		metadataRouteNN := types.NamespacedName{Name: "host-override-metadata-lb-route", Namespace: ns}
		gwNN := types.NamespacedName{Name: "same-namespace", Namespace: ns}
		...
		BackendTrafficPolicyMustBeAccepted(t, suite.Client, types.NamespacedName{Name: "host-override-header-lb-policy", Namespace: ns}, suite.ControllerName, ancestorRef)
		BackendTrafficPolicyMustBeAccepted(t, suite.Client, types.NamespacedName{Name: "host-override-metadata-lb-policy", Namespace: ns}, suite.ControllerName, ancestorRef)
		WaitForPods(t, suite.Client, ns, map[string]string{"app": "lb-backend-hostoverride"}, corev1.PodRunning, PodReady)
		...
		err := suite.Client.List(ctx, podList, &client.ListOptions{
			Namespace:     ns,
			LabelSelector: labels.SelectorFromSet(map[string]string{"app": "lb-backend-hostoverride"}),
		})
		...
		// Get service port
		svc := &corev1.Service{}
		err = suite.Client.Get(ctx, types.NamespacedName{Name: "lb-backend-hostoverride", Namespace: ns}, svc)
		...
		t.Run("header-based host override with valid pod IP should route to specific pod", func(t *testing.T) {
			...
			t.Logf("Testing host override with valid pod IP: %s, expecting pod: %s", overrideHost, expectPodName)

			expectedResponse := http.ExpectedResponse{
				Request: http.Request{
					Path: "/host-override-header",
		...
		t.Run("header-based host override with invalid pod IP should fallback to RoundRobin", func(t *testing.T) {
			// Use an invalid pod IP that's not in the service endpoints
			invalidOverrideHost := "192.168.99.99:8080"

			t.Logf("Testing host override with invalid pod IP: %s (should fallback to RoundRobin)", invalidOverrideHost)
		...
		t.Run("header-based host override without header should fallback to RoundRobin", func(t *testing.T) {
			expectedResponse := http.ExpectedResponse{
				Request: http.Request{
					Path: "/host-override-header",
		...
		t.Run("metadata-based host override should fallback to LeastRequest", func(t *testing.T) {
			expectedResponse := http.ExpectedResponse{
				Request: http.Request{
					Path: "/host-override-metadata",

...
| `clusterIP` | _string_ |  false  | ClusterIP may only be configured when Type is set to ClusterIP. This can be used to specify your own cluster IP from the service-cluster-ip-range CIDR configured for the API server, or can be set to "None" to create a headless service without a cluster IP. |

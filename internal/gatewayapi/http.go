...
func buildIRHTTP2Settings(http2Settings *egv1a1.HTTP2Settings) (*ir.HTTP2Settings, error) {
	...
	if http2Settings.ConnectionKeepalive != nil {
		...
		if http2Settings.ConnectionKeepalive.Interval != nil {
			...
			if err != nil {
				errs = errors.Join(errs, fmt.Errorf("invalid ConnectionKeepalive.Interval value %s: %w",
					*http2Settings.ConnectionKeepalive.Interval, err))
			} else {
		...
		if http2Settings.ConnectionKeepalive.Timeout != nil {
			...
			if err != nil {
				errs = errors.Join(errs, fmt.Errorf("invalid ConnectionKeepalive.Timeout value %s: %w",
					*http2Settings.ConnectionKeepalive.Timeout, err))
			} else {
		...
		if http2Settings.ConnectionKeepalive.ConnectionIdleInterval != nil {
			...
			if err != nil {
				errs = errors.Join(errs, fmt.Errorf("invalid ConnectionKeepalive.ConnectionIdleInterval value %s: %w",
					*http2Settings.ConnectionKeepalive.ConnectionIdleInterval, err))
			} else {

...
> **Note:** `source: Local` rules are applied at the listener level and cannot be scoped to a specific route. A rule on one HTTPRoute will apply to all Envoy-generated responses on the same Gateway listener, regardless of which route the request would have matched.

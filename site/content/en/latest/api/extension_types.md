...
| `accessibleMetadata` | _[ExtAuthAccessibleMetadata](#extauthaccessiblemetadata)_ |  false  |  | AccessibleMetadata defines options related to the sending of dynamic and static (xds) metadata.<br />These options define which metadata namespaces would be sent to the external authorization server.<br />Users can specify custom namespaces or well-known envoy metadata namespace (such as envoy.filters.http.header_to_metadata)<br />documented here: https://www.envoyproxy.io/docs/envoy/latest/configuration/advanced/well_known_dynamic_metadata#well-known-dynamic-metadata<br />Default: no metadata context is sent to the external authorization service. |

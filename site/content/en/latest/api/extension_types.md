...
| `ipFamily` | _[IPFamily](#ipfamily)_ |  false  | IPFamily specifies the IP family for the Envoy proxy's Gateway listener ports.<br />This setting only affects the Gateway listener ports and does not impact<br />other aspects of the Envoy proxy configuration.<br /><br />If not specified, the behavior depends on the environment:<br />- In IPv4-only environments, it defaults to IPv4.<br />- In dual-stack environments, it will use both IPv4 and IPv6 (equivalent to DualStack).<br />- In IPv6-only environments, it defaults to IPv6.<br /><br />It's recommended to explicitly set this field to ensure consistent behavior<br />across different environments. |

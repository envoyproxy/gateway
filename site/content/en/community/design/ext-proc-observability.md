...
| 6 | **Specificity-then-age wins per IR listener** using a `nameRegistry`; policies are processed in target-scope order (route-rule → route → listener → gateway), so more-specific policies claim names first. Within the same specificity level the oldest (lowest creation-timestamp) policy wins. Shadowed policies get `CustomName` cleared and an `AmbiguousDefinition` warning. The **effective policy's filter identity applies to the entire listener**: on routes where a shadowed policy's ext-proc is active, the operator expands to the effective policy's filter-state key, which is absent and produces an empty value. Under `MergeGateways` **without TLS**, name resolution is suppressed entirely: the xDS listener processing order follows gateway creation timestamps, which is independent of EEP age, making resolution non-deterministic and a potential source of filter-chain drains. `name` is silently cleared on all ext-procs on non-TLS merged listeners. Under `MergeGateways` **with TLS**, each listener gets its own filter chain and `nameRegistry`, so names are fully isolated and work normally. | Rejecting all duplicates (rejected: breaks the oldest, authoritative policy). |
...
**MergeGateways (non-TLS)**: `name` is not supported on non-TLS merged listeners. The xDS layer processes IR listeners in gateway creation-timestamp order, which is independent of EEP age used for name claiming. To avoid non-deterministic operator resolution and potential filter-chain drains, `CustomName` is cleared on all ext-procs on non-TLS merged listeners at IR build time. TLS merged listeners are unaffected — each gets its own filter chain and name registry.

...
| `aggression` | _integer_ |  false  |  | Aggression controls the speed at which the endpoint weight ramps up during the<br />slow start window. The endpoint weight is scaled by the time factor raised to<br />the power of (1 / aggression). Values greater than 1.0 result in a faster<br />initial ramp-up followed by a slower approach to the full weight (exponential),<br />while values less than 1.0 result in a slower initial ramp-up followed by a<br />faster approach to the full weight (logarithmic).<br />This is expressed as a percentage-based integer where 100 represents 1.0,<br />150 represents 1.5, 50 represents 0.5, etc. Must be greater than 0. If<br />unset, Envoy uses its default of 1.0, which produces a linear ramp-up.<br />For example:<br />- 50  => 0.5x<br />- 100 => 1.0x<br />- 150 => 1.5x<br />- 200 => 2.0x |
